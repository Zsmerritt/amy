...
-s EXPORTED_FUNCTIONS="['_amy_add_message', '_amy_add_event', '_amy_reset_sysclock', '_amy_sysclock', '_amy_process_single_midi_byte', \
	 '_amy_live_stop', '_amy_get_input_buffer', '_amy_get_output_buffer', '_amy_set_external_input_buffer', '_amy_live_start_web', '_amy_live_start_web_audioin',\
	  '_amy_start_web', '_amy_start_web_no_synths', '_sequencer_ticks', '_malloc', '_free', '_amy_bleep', '_size_of_amy_event', '_yield_synth_commands', '_amy_dump_state_to_string', '_cos_lut', \
	  '_amy_set_external_hook_context', '_amy_get_external_hook_context']" \
...
.PHONY: default all clean amy-module test web deploy-web godot-api
...
src/patches.h: $(PYTHONS) $(HEADERS_BUILD)
	cat src/amy.h  | sed -e 's@^//.*@@' | tr '\t' ' ' | egrep 'define +[^ ]+ +[.0-9-]+' | sed -e 's/\([-0-9][0-9]*\.[0-9]*\)f.*/\1/' | awk '{print $$2 "=" $$3}' | sed -e 's/=\([-.0-9]*\)[uUlLfF][uUlLfF]*$$/=\1/' > amy/constants.py
