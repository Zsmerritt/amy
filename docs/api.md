...
format, S8.23 fixed point in an `int32_t` (so 1.0 is `1<<23` â€” see
...
MicroPython on the web builds), and the Godot `AmySynth` methods â€” so
...
[**AMY JavaScript REPL**](https://shorepine.github.io/amy/repl.html) â€” write and run `amy_send()` commands in your browser.
...
| ---- | --------- | ------- | ----------- |
| `amy_external_render_hook` | `uint8_t (uint16_t osc, SAMPLE *buf, uint16_t len)` | â€” | Custom oscillator renderer for redirecting the output waveforms on an osc-by-osc level. Return 1 if handled, in which case that osc does not contribute to the normal output. |
| `amy_external_bus_postprocess_hook` | `void (uint8_t bus, SAMPLE *buf, uint16_t len)` | â€” | Custom effect processing, called at the end of each bus' effects chain (after EQ/chorus/echo/reverb), before buses are mixed to the output. `buf` is `AMY_NCHANS` sequential (non-interleaved) channel blocks of `len` `SAMPLE`s (S8.23 in `int32_t`, see above) each; modify it in place. Called every block for each bus from 0 up to the highest bus activated so far â€” so bus 0 is always processed, but a hook installed for bus 2 only starts firing once something (an osc assignment, a bus FX setting) has touched bus 2. |
| `amy_external_coef_hook` | `float (uint16_t channel)` | â€” | Provide external coefficient values (e.g. CV input). |
| `amy_external_block_done_hook` | `void (void)` | â€” | Called after each audio block is rendered. |
| `amy_external_midi_input_hook` | `void (uint8_t *bytes, uint16_t len, uint8_t is_sysex)` | â€” | Called when MIDI bytes are received. |
| `amy_external_midi_output_hook` | `void (uint8_t *bytes, uint16_t len)` | â€” | Called with every run of bytes AMY sends over MIDI out (notes, clock, sysex responses), before â€” and regardless of â€” any device interface configured in `midi`. Use it to forward AMY's MIDI output to a transport AMY doesn't drive itself, e.g. BLE MIDI. May be called from the render/sequencer task; keep it fast. |
| `amy_external_sequencer_hook` | `void (uint32_t tick_count)` | â€” | Called on each sequencer tick. |
...
| `iV`   | `synth_level` | `synth_level` | float >= 0 | Per-instrument level, default 1.0.  Scales the audio of every osc in the synth at render time (applied at the output stage, just before the pan/bus mix), independent of any osc `amp` settings.  The natural target for a channel volume control (e.g. MIDI CC 7) â€” works for every synth type, including drum kits whose per-drum oscs carry their own amps. |
